...
// predicate - takes in some arguments and returns a bool
// operator - takes in some arguments and applies an operation to them
//______________________________________________________________________________

// See cppreference.com for documentation
// for the STL algoirthms
...
// Triangles are defined by a vector of three point in any order
...
// permutation of antoher using a predicate lamdba to compare points.
void test2(){
    std::cout << "\n---Test2--------------------------------------" << std::endl;
...
// operation is deinfed within the passed lambda to each element of the sequence.
...
// This example is what is known as an erase-remove idiom and is one of
...
// element and its end iterator. 
// Used in conjunction with the vector memeber function "erase", the 
// removed elements can be erased ffrom the vector so that it only contains
...
void test4(){
    ...
    nums.erase(std::remove_if(nums.begin(), nums.end(), [](int num){
        return num % 2 == 0; // printing all odd numbers in vector
    }),
    nums.end());
...
void test5(){
    ...
    // Sorted people by age in descending order
    std::sort(people.begin(), people.end(), [](Person lhs, Person rhs){
...
void test6(){
    ...
    std::cout << std::boolalpha; // Displays: true/false

    std::vector<int> nums(10);
    std::iota(nums.begin(), nums.end(), 1); // starts at 1 and counts up
    ...
    std::cout << in_between(nums, 50, 60) << std::endl; // checks if numbers are between 50 - 60
...
class Password_Validator1 {
    private:
        char restricted_symbol {'$'}; // one character
    public:
...
class Password_Validator2 {
    private:
        std::vector<char> restricted_symbols {'!','$','+'}; // most than one character
    public:
        bool is_valid(std::string password) {
            return std::all_of(password.begin(), password.end(), [this](char character) {
...
void test7(){
    ...
    // Password_Validator1------------------------------------------------------------
    std::string password{"holywiood1$"};

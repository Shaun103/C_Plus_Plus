...
class Person {
    friend std::ostream &operator<<(std::ostream &os, const Person &p); // display Person obj
    std::string name;
    int age;
public:
    Person() = default;
    ...
    bool operator<(const Person &rhs) const {                       // always overload - how it is compared
        return this->age < rhs.age;
    }
    bool operator==(const Person &rhs) const {
        return (this->name == rhs.name && this->age == rhs.age);    // always overload
    }
};
...
void display2(const std::vector<int> &vec) {
    std::cout << "[ ";
    std::for_each(vec.begin(), vec.end(),           // for_each algorithm
        [](int x) { std::cout << x << " ";});
    std::cout << " ]" << std::endl;
}
...
void test2() {
    ...
    std::cout << "vec capacity: " << vec.capacity() << std::endl; // limit, exceeds the size, it will grow
    
    vec.push_back(6);   // adding 6 to vector 
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl; // capacity is doubled
    
    vec.shrink_to_fit();    // C++11    // reduce size 
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;
    
    vec.reserve(100);       // wont expand till it hits 101
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;
...
void test4() {
    ...
    Person p1 {"Larry ", 18};
    display(stooges);       // displays nothing
    
    stooges.push_back(p1);  // adds p1 to stooges vector
    display(stooges);
    
    stooges.push_back(Person{"Moe", 25}); // adds p1 to stooges vector
    display(stooges);
...
void test5() {
    ...
    stooges.pop_back();  // removes element from back of the vector
    display(stooges);
}
...
void test6() {
    ...
    display(vec);
    vec.erase(vec.begin(), vec.begin() + 2);  // gets rid of the first two elements
    display(vec);
    ...
    auto it = vec.begin();
    while (it != vec.end()) {
        if (*it % 2 == 0)   // if even, erase the element
            vec.erase(it);  
        else
...
void test7() {
    ...
    std::vector<int> vec2 {10,20,30,40};    // size = 4
...
void test8() {
    ...
    display(vec1);
    std::sort(vec1.begin(), vec1.end());  // sorth method 
    display(vec1);

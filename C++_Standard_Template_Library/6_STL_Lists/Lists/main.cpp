...
void test3() {
    ...
    l.resize(10);   // fills space with default '0'
    display(l);
...
void test4() {
    ...
    auto it = std::find(l.begin(), l.end(), 5); // 5 is iterator  
    
    if (it != l.end()) {
        l.insert(it, 100);  // insets before 
    }
    ...
    // ADVANCED function - placing the iterator -4 movements
...
void test5() {
    ...
    auto it2 = std::find(stooges.begin(), stooges.end(), Person{"Larry", 18});
    if (it2 != stooges.end())
        stooges.emplace(it2, "Junior", 18);
    display(stooges);
}
